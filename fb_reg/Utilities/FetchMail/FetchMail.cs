...
namespace fb_reg.Utilities
{
    public static class FetchAllMail
    {
        public static int CheckHasMail(bool tempmail)
        {
            ...
            MailObject resp = CacheServer.ForceAddMailServerCache(mail);
        ...
        public static void FetchGmail(DeviceObject device, bool tempmail)
        {
            try
            {
                ...
                if (listmail != null && listmail.Count > 0 )
                {
                    try
                    {
                        PublicData.dataGridView.Rows[device.index].Cells[13].Value = listmail[0].source + "-" + listmail[0].email + "-b:" + listmail[0].balanceAfter;
                        FetchController.SetState(FetchState.WaitingServer);
                        MailObject resp = new MailObject();
                        for (int k = 0; k < listmail.Count; k++) {
                            resp = CacheServer.ForceAddMailServerCache(listmail[k]);
                        }
                        
                        PublicData.PublicmaxMaillabel.Text = listmail.Count + " -" +  listmail[0].email + "-" + listmail[0].source + "-" + listmail[0].balanceAfter + "-" + PublicData.FetchMailLog;
                        if (resp != null)
                        {
