...
namespace fb_reg
{
    class LDPlayer
    {
        ...
        public List<Info_Devices> GetDevices2_Running()
        {
            ...
            {
                int j = 0;
                List<string> list_adb_id = Device.GetDevices();
                List<Info_Devices> listLDPlayer = new List<Info_Devices>();
        ...
        public void Tap_Img(string deviceID, Bitmap ImgFind)
        {
            ...
            var screen = Device.ScreenShoot(deviceID);
            var Point = Device.FindOutPoint(screen, bm);
            if (Point != null)
            {
                Device.Tap(deviceID, Point.Value.X, Point.Value.Y);
                return;
            }
        ...
        {
            Device.ExecuteCMD(string.Format("adb -s {0} shell settings put global http_proxy {1}:{2}", deviceID, ip_proxy, port_proxy));
        }

        public void Remove_Proxy(string deviceID)
        {
            Device.ExecuteCMD(string.Format("adb -s {0} shell settings put global http_proxy :0", deviceID));
        }
    }
}
